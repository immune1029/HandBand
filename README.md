...
- ImageSerchingAndDetect(FPGA)  
   ...
   - 좌표 추출 과정은 순수 하드웨어(FPGA) 로직으로 구성되어, Basys3버전의 경우 외부 CPU 개입 없이 고속으로 처리된다. Zybo버전의 경우 SCCB와 VDMA 설정까지는 CPU가 사용된다(FHD 출력을 위해서는 Zybo의 DRAM에 접근해야하며 그 과정에서 VDMA모듈이 사용된다).
   - 모듈 소개
